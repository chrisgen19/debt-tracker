    ...
    <div className="min-h-svh bg-background text-foreground">
      ...
      <main className="mx-auto max-w-[1440px] px-4 pb-[calc(env(safe-area-inset-bottom)+7rem)] pt-7 sm:px-6 lg:px-10 lg:pb-12">
        ...
        <section className="mb-6 grid grid-cols-1 gap-6 sm:grid-cols-[minmax(0,1fr)_auto] xl:grid-cols-[minmax(0,1.55fr)_minmax(0,.85fr)]">
          ...
          <div className="order-2 sm:order-4 sm:col-span-2">
            <SummaryCarousel
              label="Balance and monthly movement"
              railClassName="sm:mx-0 sm:grid sm:snap-none sm:grid-cols-1 sm:gap-6 sm:overflow-visible sm:px-0 sm:pb-0 xl:grid-cols-[minmax(0,1.55fr)_minmax(0,.85fr)]"
              items={[
                { key: "balance", label: "All-time balance", className: "sm:order-2", node: <BalanceCard className="h-full" currentUser={currentUser} partner={partner} summary={summary} currency={currency} /> },
                { key: "movement", label: "Monthly movement", className: "sm:order-1", node: <MovementCard className="h-full" month={month} chart={chart} currency={currency} /> },
              ]}
            />
          </div>
